...
from google.api_core.exceptions import (
    NotFound, AlreadyExists, InvalidArgument, PermissionDenied, GoogleAPIError
)
...
class TestFileSearchStoreManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patcher = patch('security_chatbot.utils.api_client.GeminiClientManager.get_client', return_value=self.mock_client)
    ...
    def test_create_store_success(self):
        ...
        mock_created_store = types.FileSearchStore(name=mock_store_name, display_name=mock_display_name)
    ...
    def test_create_store_already_exists(self):
        ...
        self.mock_file_search_stores.create.side_effect = AlreadyExists("Store already exists.")
    ...
    def test_create_store_api_error(self):
        ...
        self.mock_file_search_stores.create.side_effect = InvalidArgument("Invalid display name.")
    ...
    def test_get_store_success(self):
        ...
        mock_store_name = "fileSearchStores/test-store-123"
        mock_display_name = "My Test Store"
        mock_retrieved_store = types.FileSearchStore(name=mock_store_name, display_name=mock_display_name)
    ...
    def test_get_store_api_error(self):
        ...
        self.mock_file_search_stores.get.side_effect = PermissionDenied("Permission denied.")
    ...
    def test_list_stores_success_with_items(self):
        ...
        mock_store_1 = types.FileSearchStore(name="fileSearchStores/s1", display_name="Store 1")
        mock_store_2 = types.FileSearchStore(name="fileSearchStores/s2", display_name="Store 2")
        self.mock_file_search_stores.list.return_value = iter([mock_store_1, mock_store_2])
    ...
    def test_list_stores_api_error(self):
        ...
        self.mock_file_search_stores.list.side_effect = PermissionDenied("Access denied.")
    ...
    def test_delete_store_success(self):
        ...
        self.mock_file_search_stores.delete.return_value = None  # delete는 반환값이 없음

        result = self.manager.delete_store(store_name=mock_store_name)

        self.assertTrue(result)
        self.mock_file_search_stores.delete.assert_called_once_with(name=mock_store_name)

    def test_delete_store_not_found(self):
        """스토어 삭제 시 찾을 수 없는 경우 테스트"""
        self.mock_file_search_stores.delete.side_effect = NotFound("Store not found for deletion.")

        result = self.manager.delete_store(store_name="fileSearchStores/non-existent-to-delete")
    ...
    def test_delete_store_api_error(self):
        ...
        self.mock_file_search_stores.delete.side_effect = PermissionDenied("Cannot delete this store.")

        result = self.manager.delete_store(store_name="fileSearchStores/forbidden-to-delete")
