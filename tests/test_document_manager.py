...
from google.api_core.exceptions import (
    NotFound, InvalidArgument, PermissionDenied, GoogleAPIError,
    InternalServerError, ServiceUnavailable, ResourceExhausted
)
...
class TestDocumentManager(unittest.TestCase):
    ...
    def test_upload_file_success(self):
        file_path = self._create_temp_file("test.pdf", 1024)
        mock_uploaded_file = types.File(name="files/test-file-123", display_name="test.pdf")
        ...
        self.assertEqual(result.name, "files/test-file-123")
    ...
    def test_wait_for_indexing_success(self):
        ...
        result = self.manager.wait_for_indexing(operation_name, timeout=10, poll_interval=1)
    ...
    def test_wait_for_indexing_timeout(self, mock_sleep):
        ...
        self.mock_operations.get.return_value = mock_operation
        result = self.manager.wait_for_indexing(operation_name, timeout=5, poll_interval=2)
