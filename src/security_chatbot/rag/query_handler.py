...
def format_response(response: types.GenerateContentResponse, citations: List[str]) -> Dict[str, Any]:
    """
...
def query_with_rag(query: str, store_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if formatted_response["success"]:
            ...
            logger.warning(f"RAG 쿼리 실패: {formatted_response.get('error', '알 수 없는 오류')}")
